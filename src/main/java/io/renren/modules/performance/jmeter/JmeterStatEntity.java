...
public class JmeterStatEntity {
     ...
     * 当前是否正在运行
     */
    private Integer runStatus = PerformanceTestUtils.RUNNING;
    ...
    public JmeterStatEntity(Long fileId, Long fileIdZero) {
        if (fileIdZero != null) {// 分布式情况下
            ...
            statMap = PerformanceTestUtils.samplingStatCalculator4File.get(fileIdZero);
        } else {// 单机模式下
            this.fileId = fileId;
            statMap = PerformanceTestUtils.samplingStatCalculator4File.get(fileId);
        }

        // StressTestUtils.jMeterEntity4file 中保存的都是真实的脚本文件信息
        jmeterRunEntity = PerformanceTestUtils.jMeterEntity4file.get(fileId);
        if (jmeterRunEntity != null) {
            runStatus = jmeterRunEntity.getRunStatus();
        } else {
            // jmeterRunEntity 为null 说明是初始化状态，没有执行过。
            runStatus = PerformanceTestUtils.INITIAL;
        }
    }
