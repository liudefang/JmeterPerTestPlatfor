package io.renren.modules.performance.jmeter;
...
public class JmeterResultCollector extends ResultCollector {
    ...
    public JmeterResultCollector(PerformanceCaseFileEntity perTestFile) {
        samplingStatCalculatorMap = new HashMap<>();
        this.perTestFile = perTestFile;
        if (StringUtils.isNotEmpty(perTestFile.getSlaveStr())) {//分布式压测
            PerformanceTestUtils.jMeterStatuses.put(SLAVE_NEED_REPORT, perTestFile.getReportStatus().toString());
            PerformanceTestUtils.jMeterStatuses.put(SLAVE_NEED_CHART, perTestFile.getWebchartStatus().toString());
            //对于分布式，不再按照脚本文件来区分前端监控，分布式压测不支持master同时压测多个脚本文件的前端区分监控。
            PerformanceTestUtils.samplingStatCalculator4File.put(0L, samplingStatCalculatorMap);
        } else {
            PerformanceTestUtils.samplingStatCalculator4File.put(perTestFile.getFileId(), samplingStatCalculatorMap);
        }
    }
    ...
    public void sampleOccurred(SampleEvent sampleEvent) {
        if (perTestFile != null && StringUtils.isEmpty(perTestFile.getSlaveStr())){ //单节点压测
            ...
            if (PerformanceTestUtils.NEED_WEB_CHART.equals(perTestFile.getWebchartStatus())) {
                addSample(sampleEvent);
            }
        } else {//分布式压测
            if (PerformanceTestUtils.NEED_REPORT.toString().
                    equals(PerformanceTestUtils.jMeterStatuses.get(SLAVE_NEED_REPORT))) {
                super.sampleOccurred(sampleEvent);
            }
            if (PerformanceTestUtils.NEED_WEB_CHART.toString().
                    equals(PerformanceTestUtils.jMeterStatuses.get(SLAVE_NEED_CHART))) {

                samplingStatCalculatorMap = PerformanceTestUtils.samplingStatCalculator4File.get(0L);
